...
/**
 * UncaughtException处理类。需要在AndroidManifest.xml的Application中注册，
 * 当程序发生Uncaught异常的时候。由该类来接管程序。并记录发送错误报告（本地sdcard 或 远端服务器）.
 */
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private String getFilePath() {
        ...
        if (isSDCardExist && isRootDirExist) {
            ...
            file_dir =
                    CrashApplication.getInstance().getFilesDir()
                    .getAbsolutePath() + "/crashlog/";
        }
        return file_dir;
    }
